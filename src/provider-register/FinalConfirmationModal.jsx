...
export default function FinalConfirmationModal({ isOpen, formData, onConfirm, onCancel, error, isSubmitting }) {
    const dataPoints = [
        ...
        { label: 'Full Name', value: `${formData.firstName} ${formData.lastName}`, icon: <Check className="w-3 h-3 text-emerald-500" /> },
        { label: 'Discretion', value: formData.discretion ? (formData.discretion.substring(0, 50) + (formData.discretion.length > 50 ? '...' : '')) : 'Not provided', icon: <Check className="w-3 h-3 text-emerald-500" /> },
    ];
    return (
        <AnimatePresence>
            {isOpen && (
